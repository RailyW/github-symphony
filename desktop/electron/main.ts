...
function mergeSettingsWithDefaults(stored: Record<string, unknown>): Record<string, unknown> {
  ...
  if (shouldPreserveLegacyApprovalPolicy(stored)) {
    merged.codex = {
      ...((isPlainObject(merged.codex) ? merged.codex : {}) as Record<string, unknown>),
      approval_policy: legacyGranularApprovalPolicy(),
    };
  }
...
// 函数说明：已有 settings 若没有显式 approval_policy，沿用保守策略，避免升级时静默扩大权限。
function shouldPreserveLegacyApprovalPolicy(value: Record<string, unknown>): boolean {
...
// 函数说明：返回可写入 settings 的独立 legacy approval policy 对象，避免共享引用被修改。
...
// 函数说明：旧安装若仍保存短提示词，则升级为当前随包的 PR 前自治 prompt。
function migrateLegacyPromptTemplate(settings: Record<string, unknown>): void {
  const promptTemplate = settings.prompt_template;
  if (
    typeof promptTemplate !== "string"
    || normalizePromptForComparison(promptTemplate) !== normalizePromptForComparison(LEGACY_BRIEF_PROMPT_TEMPLATE)
  ) {
    return;
  }
  settings.prompt_template = readBundledPromptTemplate();
}

// 函数说明：只为迁移比较规整换行和首尾空白，避免改写用户自定义 prompt。
