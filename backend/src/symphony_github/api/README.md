...
- `GET /api/v1/logs/query?level=&event_type=&identifier=&q=&cursor=`
...
Logs API 读取后端 JSONL 日志目录中的结构化日志，供桌面端 Logs 页面做过滤和诊断包导出。导出包会包含最近日志、当前 state、配置摘要和运行环境摘要，所有 token、Authorization header 和 PAT 都必须在写入和导出前脱敏。
