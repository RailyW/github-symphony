...
DEFAULT_PROMPT_TEMPLATE = "\n".join(
    [
        ...
        "",
        (
            "你在 non-interactive runner（无人值守）的隔离工作区内执行完整实现循环。"
            "runner 无法接收 `ok` / `行` / `continue` / “确认后继续”等人工输入；"
            "不要要求人工确认后才继续。"
        ),
        "",
        (
            "调度器只负责派发任务、准备工作区、注入 GitHub 工具和记录事件；"
            "代码流转动作由你根据本 prompt、token 权限、GitHub tools 模式和 "
            "Project Status 执行。在 GitHub Project item 仍处于 active 状态且工作"
            "仍限定在当前 issue scope 内时，创建/复用任务分支、修改代码、运行验证，"
            "以及 commit、push task branch、create/update PR 已授权；不得等待人工"
            "确认后再 commit、push 或创建/更新 PR。"
        ),
        "",
        "### 通用规则",
        "",
        ...
        (
            "2. 使用单个 issue comment 作为 `## Codex Workpad`。如果已存在 Workpad，"
            "就更新它；不要新建多个进度评论。"
        ),
        ...
        "",
        (
            "- `Todo`：先使用 GitHub 工具把 Project Status 移到 `In Progress`，"
            "然后创建或更新 `## Codex Workpad`，再开始复现、计划和实现。"
        ),
        (
            "- `In Progress` / `Rework`：完成复现、计划、实现和验证。创建或复用"
            "任务分支，保持分支基于最新默认分支；按逻辑提交 commit，push 到远端，"
            "并创建或更新一个 PR。"
        ),
        (
        ...
        (
            ...
            "等待人工审批或把状态移到 `Rework` / `Merging`。"
        ),
        (
            "- `Merging`：这是唯一允许自动 merge 的 active land 状态。只执行合并前"
            "检查和 land 流程：确认 PR 已获人工批准、checks green、分支已同步、"
            "必要验证仍通过，然后使用默认 squash merge 合并，并把 Project Status 移到 `Done`。"
        ),
        "",
        "### PR feedback sweep",
        "",
        ...
        (
            "- 不要使用 PR body closing keywords 自动关闭 issue，也不要自动关闭 issue；"
            "任务结束以 GitHub Project Status `Done` 为准。"
        ),
...
def default_app_settings() -> Dict[str, Any]:
    return {
        ...
        "codex": {
            ...
            "model": "gpt-5.5",
            "approval_policy": _default_high_trust_approval_policy(),
...
# 函数说明：返回新建 App settings 使用的 PR 前全自动高信任 preset。
def _default_high_trust_approval_policy() -> Dict[str, Any]:
    return {"preset": "high-trust"}
